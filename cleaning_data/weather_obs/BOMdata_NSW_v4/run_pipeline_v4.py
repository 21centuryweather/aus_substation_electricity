...
OUT_ROOT = DATA_ROOT / "BOM_NSW_weather_processed_v4"   # <-- v4 output
META_FILE = RAW_ROOT / "HM01X_StnDet_MASTER_v4.txt"
...
def process_all_stations():
    ...
    for station_dir in sorted(d for d in RAW_ROOT.iterdir() if d.is_dir()):
        ...
        try:
            # FIXED: correct glob pattern (no stray spaces)
            ...
            df["station_id"] = str(station_id).zfill(6)
