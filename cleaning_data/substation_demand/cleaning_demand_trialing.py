...
__title__ = "Clean Melbourne and Brisbane 1/2 hourly electricity demand data"
...
BASE = Path("/g/data/ng72/pv3484/aus_substation_electricity/data")
RAW = BASE / "raw_data"
CLEAN = BASE / "cleaned_data"

BRIS_DEMAND = RAW / "QLD_demand"
MELB_DEMAND = RAW / "VIC_demand"

# ------
#oshome=os.getenv('HOME') #16
#projpath = f'.'
#datapath = f'{projpath}/data'
#obspath =  f'{projpath}/data/BOMdata'
#plotpath = f'{projpath}/figures' #21
...
# ---------
# select state domains - not needed for now, commented out of module
# domains = ['vic','nsw','qld','wa','tas','sa', 'act']
# domains = ['nsw']

#linearly fill gaps max size - not needed
#fill_gaps = False

#def get_demand_data(suppliers,domain,sdate,edate): #lines 37 - 63
    #commenting out as substation data is not organised by supplier, domain or BOM
...
#def read_bom_half_hourly(): lines 65 - 84
    # commenting out as I am not using BOM half hourly temp data and this module's purpose is to clean substation data, not process it and combine with weather
#    '''
...
#def tidy_aws_columns(df): lines 86 - 104
    #commenting out as this module isn't cleaning or using weather observation data
...
#def get_substation_data(filename): lines 106 - 143
#    '''
...
def load_supplier_demand_and_metadata(supplier_csv, metadata_df):
    ...
    Args:
        ...
        metadata_df (DataFrame): full DNSP metadata table
    ...
    # 3. Extract substation IDs from NEAR column names
    ...
    # 4. Filter metadata to these IDs
...
def load_state_demand_and_metadata(state, state_dir, metadata_path):
    ...
    for supplier_folder in Path(state_dir).iterdir():
        if supplier_folder.is_dir():
            # Find CSVs inside this supplier folder
...
#def get_domain_info(domain): #lines 157 - 184
    #commented out as this uses variable domain which I don't use
#    '''
...
def get_supplier_info_from_csv(supplier_csv, metadata_path, state):
    """
    Extract metadata (info) for the substations present in a supplier's
    CSIRO NEAR demand CSV.

    Args:
        supplier_csv (str or Path): path to one supplier's demand CSV
        metadata_path (str or Path): path to DNSP_Zone_Substation_Characteristics.csv
        state (str): "QLD" or "VIC"

    Returns:
        info (DataFrame): metadata for substations in this supplier
    """

    # ---------------------------------------------------------
    # 1. Load full DNSP metadata
    # ---------------------------------------------------------
    meta = pd.read_csv(metadata_path)

    # Filter to the correct state
    meta_state = meta[meta["State"] == state].copy()

    # ---------------------------------------------------------
    # 2. Load supplier CSV to extract substation IDs
    # ---------------------------------------------------------
    df = pd.read_csv(supplier_csv)

    # Extract NEAR-style IDs from column names
    # "'ABY'|'Amberley'|ActivePower" → "ABY"
    raw_cols = df.columns[3:]   # skip StartDeliveryTime, EndDeliveryTime, UtilityName
    ids = [col.split("'")[1] for col in raw_cols]

    # ---------------------------------------------------------
    # 3. Filter metadata to only these IDs
    # ---------------------------------------------------------
    info = meta_state[meta_state["ID"].isin(ids)].copy()

    # Set ID as index for convenience
...
def select_sites(info, area_min=0, res_min=0, res_max=1, com_min=0, com_max=1,
    ...
    Args:
        info (DataFrame): NEAR metadata with columns like 'Residential', 'Commercial', etc.
        area_min (float): minimum substation area (km²)
        res_min/res_max (float): residential fraction bounds
        com_min/com_max (float): commercial fraction bounds
        ind_min/ind_max (float): industrial fraction bounds
        farm_max (float): max primary production fraction

    Returns:
        List of substation IDs that meet all criteria.
    """

    filtered = info[
        (info["Zone Substation Area (km²)"] > area_min) &
